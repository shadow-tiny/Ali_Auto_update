### 1.3 更新内容：
### 修复了1.2中由于网盘链接失效而导致中断的bug
### 并利用推送消息通知用户失效的链接信息
### Updated on June 14     ----by wsubset

# 对于挂载阿里云盘的播放器，实现自动更新视频，并重命名为原产国名称以及季度剧集的自动计算
# 实现更快速的更新,且无需创建命名一致的文件夹，会按照txt文档命名自动创建命名一致的文件夹
...
def creat_dict(txt_path):
    with open(txt_path, "r") as file:  # 此处填写你自己的txt文件路径
...
def find_path(file_name):
    file = ali.get_folder_by_path('****/****/****')  # 填写你希望开始路径，减少查找路径时间
    path_sub = local_tree(file_name, file.file_id)
    if path_sub is not None:
        path = '****/****/****' + path_sub  # 此处与上方同时修改
        return path
    else:
        print("----------目前未找到相关文件夹----------")
        print("----------以下为您新建该目录至默认路径下(未设置即为根目录)----------")
        ali.create_folder(file_name, file.file_id)
...
def delete(full_name):
    d_num = []
    with open("*****你的txt文件在本地的地址*****", 'r') as r:
        lines = r.readlines()
        for line in lines:
    ...
    ### 删除日志对应的记录
    with open("*****你的txt文件在本地的地址*****", 'w') as w:
        for l in lines:
...
def share_main(search_url, all_files):
    ...
    ### flag用于记录tree_share中的失效标记
    flag = tree_share(share_token, share_parent_id, all_files)
    if flag is False:
        share_token = False  ### 使用同一个返回值（share_token）代出，使用一个不可能存在的值赋返回值
    to_file(all_files)
    return share_token
...
def f_main(full_name, name_url_dict, flag_1, flag_2):
    ...
    url_check(search_url)
    share_token = share_main(search_url, all_files)
    if share_token is False:
        msg = "%s 链接已失效，请更换！" % (name)
        yuntui_push(msg)
    else:
        print('----------获取共享文档完成----------')
        all_files_sorted = file_sort(all_files)
        ...
        if length_file == length_share:
            ...
            ep = save(file_local, all_files_sorted, share_token, length_file, length_share, ss, name, flag_1, flag_2,
                      full_name)
            check_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
            if flag_1 == 0:
...
if __name__ == '__main__':
    ...
    # name_url_dict_1 = {}
    # name_url_dict_0 = {}
    name_url_dict_1 = creat_dict("*****你的txt文件在本地的地址*****")
    name_url_dict_0 = creat_dict("*****你的txt文件在本地的地址*****")
    # 两部字典差集
